...
import { ApiConnectionError, ApiRequestError, FideliOSApiClient } from "../client/http.js";

describe("FideliOSApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new FideliOSApiClient({
    ...
    expect(headers["x-fidelios-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new FideliOSApiClient({ apiBase: "http://localhost:3100" });
