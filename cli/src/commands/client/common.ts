...
import { ApiRequestError, FideliOSApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to FideliOS config file")
    .option("-d, --data-dir <path>", "FideliOS data directory root (isolates state from ~/.fidelios)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the FideliOS API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.FIDELIOS_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.FIDELIOS_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.FIDELIOS_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set FIDELIOS_COMPANY_ID, or set context profile companyId via `fidelios context set`.",
    );
  }

  const api = new FideliOSApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.FIDELIOS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.FIDELIOS_SERVER_PORT || "");
