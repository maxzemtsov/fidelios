...
const JWT_SECRET_ENV_KEY = "FIDELIOS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by FideliOS CLI commands",
...
export function resolveFideliOSEnvFile(configPath?: string): string {
...
export function loadFideliOSEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeFideliOSEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readFideliOSEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeFideliOSEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeFideliOSEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readFideliOSEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
