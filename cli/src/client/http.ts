...
export class FideliOSApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-fidelios-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the FideliOS API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the FideliOS server is not running, the configured URL is wrong, or the request is being blocked before it reaches FideliOS.",
    "",
    "Try:",
    "- Start FideliOS with `pnpm dev` or `pnpm fidelios run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If FideliOS is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`FIDELIOS_API_URL\`.`,
  );
  return lines.join("\n");
}
