...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("fidelios-worktree-enabled") !== "true") return null;

  const name = readMetaContent("fidelios-worktree-name");
  const color = normalizeHexColor(readMetaContent("fidelios-worktree-color"));
