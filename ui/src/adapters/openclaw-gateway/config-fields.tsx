...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="FideliOS API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "fideliosApiUrl",
                  String(config.fideliosApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "fideliosApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://fidelios.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "fidelios"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="fidelios"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. FideliOS persists a device key during onboarding so pairing approvals
