...
Use when:
- You want FideliOS to run Pi (the AI coding agent) locally as the agent runtime
...
- FideliOS requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/fidelioss/ and resumed with --session.
