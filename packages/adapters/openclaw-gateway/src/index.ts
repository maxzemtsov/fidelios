...
Use when:
- You want FideliOS to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the FideliOS server.
...
- workspaceRuntime (object, optional): desired runtime service intents; FideliOS forwards these in a standardized fidelios.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- fideliosApiUrl (string, optional): absolute FideliOS base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default fidelios)

Standard outbound payload additions:
- fidelios (object): standardized FideliOS context added to every gateway agent request
- fidelios.workspace (object, optional): resolved execution workspace for this run
- fidelios.workspaces (array, optional): additional workspace hints FideliOS exposed to the run
- fidelios.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
