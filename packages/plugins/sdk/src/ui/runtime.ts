...
type GlobalBridge = typeof globalThis & {
  __fideliosPluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__fideliosPluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `FideliOS plugin UI runtime is not initialized for "${name}". ` +
