...
export const pluginManifestV1Schema = z.object({
  ...
  minimumFideliOSVersion: z.string().regex(
    /^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/,
    "minimumFideliOSVersion must follow semver (e.g. 1.0.0)",
  ).optional(),
  ...
  if (
    ...
    && manifest.minimumFideliOSVersion
    && manifest.minimumHostVersion !== manifest.minimumFideliOSVersion
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "minimumHostVersion and minimumFideliOSVersion must match when both are declared",
