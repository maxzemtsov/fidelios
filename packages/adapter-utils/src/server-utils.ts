...
const FIDELIOS_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface FideliOSSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: FideliOSSkillEntry[];
...
> {
  ...
  return {
    ...
    originLabel: "Managed by FideliOS",
    readOnly: false,
  };
}
...
export function buildFideliOSEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.FIDELIOS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.FIDELIOS_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.FIDELIOS_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.FIDELIOS_API_URL = apiUrl;
  return vars;
}
...
export async function resolveFideliOSSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...FIDELIOS_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listFideliOSSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<FideliOSSkillEntry[]> {
  const root = await resolveFideliOSSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
...
function normalizeConfiguredFideliOSRuntimeSkills(value: unknown): FideliOSSkillEntry[] {
...
export async function readFideliOSRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<FideliOSSkillEntry[]> {
  const configuredEntries = normalizeConfiguredFideliOSRuntimeSkills(config.fideliosRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listFideliOSSkillEntries(moduleDir, additionalCandidates);
}

export async function readFideliOSSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readFideliOSSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.fideliosSkillSync;
...
function canonicalizeDesiredFideliOSSkillReference(
  reference: string,
...
export function resolveFideliOSDesiredSkillNames(
  config: Record<string, unknown>,
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredFideliOSSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeFideliOSSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.fideliosSkillSync;
  const current =
  ...
  next.fideliosSkillSync = current;
  return next;
}
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the FideliOS server itself is started from
    // within a Claude Code session (e.g. `npx fidelios run` in a terminal
