...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "fidelios", LOGNAME: "fidelios", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "fidelios" }),
      },
    );

    expect(tokens).toEqual(["fidelios", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "fidelios" }, {
        userInfo: () => ({ username: "fidelios" }),
      });

      expect(tokens).toEqual(["fidelios", "custom-token"]);
    } finally {
