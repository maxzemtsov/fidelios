...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("FideliOS OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested FideliOS base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("fideliosApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.fideliosApiUrl");
    expect(text).toContain("~/.openclaw/workspace/fidelios-claimed-api-key.json");
    expect(text).toContain("FIDELIOS_API_KEY");
