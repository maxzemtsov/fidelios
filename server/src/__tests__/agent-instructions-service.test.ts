...
describe("agent instructions service", () => {
  const originalFideliOSHome = process.env.FIDELIOS_HOME;
  const originalFideliOSInstanceId = process.env.FIDELIOS_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalFideliOSHome === undefined) delete process.env.FIDELIOS_HOME;
    else process.env.FIDELIOS_HOME = originalFideliOSHome;
    if (originalFideliOSInstanceId === undefined) delete process.env.FIDELIOS_INSTANCE_ID;
    else process.env.FIDELIOS_INSTANCE_ID = originalFideliOSInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const fideliosHome = await makeTempDir("fidelios-agent-instructions-home-");
    const externalRoot = await makeTempDir("fidelios-agent-instructions-external-");
    cleanupDirs.add(fideliosHome);
    cleanupDirs.add(externalRoot);
    process.env.FIDELIOS_HOME = fideliosHome;
    process.env.FIDELIOS_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("fidelios-agent-instructions-new-external-");
    cleanupDirs.add(fideliosHome);
    cleanupDirs.add(externalRoot);
    process.env.FIDELIOS_HOME = fideliosHome;
    process.env.FIDELIOS_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("fidelios-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const fideliosHome = await makeTempDir("fidelios-agent-instructions-recover-");
    cleanupDirs.add(fideliosHome);
    process.env.FIDELIOS_HOME = fideliosHome;
    process.env.FIDELIOS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const fideliosHome = await makeTempDir("fidelios-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("fidelios-agent-instructions-stale-root-");
    cleanupDirs.add(fideliosHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const fideliosHome = await makeTempDir("fidelios-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("fidelios-agent-instructions-heal-write-stale-");
    cleanupDirs.add(fideliosHome);
    cleanupDirs.add(staleRoot);
    process.env.FIDELIOS_HOME = fideliosHome;
    process.env.FIDELIOS_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const fideliosHome = await makeTempDir("fidelios-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("fidelios-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(fideliosHome);
    cleanupDirs.add(staleRoot);
    process.env.FIDELIOS_HOME = fideliosHome;
    process.env.FIDELIOS_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const fideliosHome = await makeTempDir("fidelios-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("fidelios-agent-instructions-partial-root-");
    cleanupDirs.add(fideliosHome);
    cleanupDirs.add(staleRoot);
    process.env.FIDELIOS_HOME = fideliosHome;
    process.env.FIDELIOS_INSTANCE_ID = "test-instance";
