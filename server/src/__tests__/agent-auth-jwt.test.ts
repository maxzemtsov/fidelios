...
describe("agent local JWT", () => {
  const secretEnv = "FIDELIOS_AGENT_JWT_SECRET";
  const ttlEnv = "FIDELIOS_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "FIDELIOS_AGENT_JWT_ISSUER";
  const audienceEnv = "FIDELIOS_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "fidelios";
    process.env[audienceEnv] = "fidelios-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
