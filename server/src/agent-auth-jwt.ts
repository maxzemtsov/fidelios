...
function jwtConfig() {
  const secret = process.env.FIDELIOS_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.FIDELIOS_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.FIDELIOS_AGENT_JWT_ISSUER ?? "fidelios",
    audience: process.env.FIDELIOS_AGENT_JWT_AUDIENCE ?? "fidelios-api",
  };
}
