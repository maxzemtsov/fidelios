...
ClipHub is the public registry where people share, discover, and download FideliOS company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to FideliOS what a package registry is to a programming language. FideliOS already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in FideliOS — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own FideliOS instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the FideliOS spec. It contains:
...
**Install (fresh start):**
```
fidelios install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local FideliOS instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within FideliOS, export your company as a template, then publish:

```
fidelios export --template my-company
fidelios publish cliphub my-company
```
...
ClipHub is a **separate service** from FideliOS itself. FideliOS is self-hosted; ClipHub is a hosted registry that FideliOS instances talk to.
...
| **FideliOS CLI** | `fidelios install`, `fidelios publish`, `fidelios cliphub sync` — built into FideliOS |
| **FideliOS UI** | "Browse ClipHub" panel in the FideliOS web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with FideliOS) |
| Backend | TypeScript + Hono (consistent with FideliOS) |
| Database | PostgreSQL |
...
4. Run `fidelios install cliphub:acme/lean-saas-shop`
5. FideliOS creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in FideliOS until it works well
2. Export: `fidelios export --template my-agency`
3. Publish: `fidelios publish cliphub my-agency`
...
3. Install just that agent: `fidelios install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to FideliOS

ClipHub is **not required** to use FideliOS. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to FideliOS what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `fidelios install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in FideliOS web UI
- [ ] `fidelios cliphub sync` for bulk publishing
