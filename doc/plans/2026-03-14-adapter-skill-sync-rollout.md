...
This document defines the rollout plan for adapter-wide skill support in FideliOS.
...
- FideliOS stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

FideliOS currently has these adapters:
...
1. Desired skills live in FideliOS for every adapter.
...
These adapters have a stable local skills directory that FideliOS can read and manage.
...
These adapters do not have a meaningful FideliOS-owned persistent install state.
...
- desired skills stored in FideliOS
...
- runtime already injects FideliOS skills into `~/.cursor/skills`
...
- runtime already injects FideliOS skills into `~/.gemini/skills`
...
- runtime already injects FideliOS skills into `~/.pi/agent/skills`
...
- FideliOS can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects FideliOS skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- FideliOS may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed FideliOS skills that are clearly marked as FideliOS-managed
...
- FideliOS stores desired skills only
...
- avoid destructive wording unless FideliOS can prove a skill is FideliOS-managed
