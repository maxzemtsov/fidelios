...
This document defines the next-stage plan for FideliOS company import/export.

The core shift is:

- move from a FideliOS-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside FideliOS.
...
FideliOS already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just FideliOS
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `fidelios.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.fidelios.yaml` sidecar for high-fidelity FideliOS-specific details
...
11. adapter-aware skill sync surfaces so FideliOS can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point FideliOS at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a FideliOS sidecar.
...
FideliOS must not redefine `SKILL.md`.
...
- FideliOS-specific fidelity lives in `.fidelios.yaml`
- FideliOS may resolve and install `SKILL.md` packages, but it must not require a FideliOS-only skill format
...
### 5.4 Base Package Vs FideliOS Extension
...
- base package:
  ...
  - no FideliOS-only runtime fields by default
- FideliOS extension:
  - `.fidelios.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by FideliOS tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`fidelios.manifest.json` is not part of the future package direction.
...
FideliOS import/export should support these entity kinds:
...
- FideliOS can treat “this set of agents came from team package X” as the imported-team model
...
FideliOS should support source references in package metadata with:
...
FideliOS should define a new adapter surface area around skills:
...
Planned FideliOS behavior:

- if an adapter supports read, FideliOS should show current skills in the UI
- if an adapter supports write, FideliOS should let the user enable/disable imported skills
- if an adapter supports sync, FideliOS should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.fidelios.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.fidelios.yaml` alongside the base package
...
## 10. Storage Model Inside FideliOS
...
FideliOS should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `fidelios company export <company-id> --out <path>`
- `fidelios company import <path-or-url> --dry-run`
- `fidelios company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in FideliOS storage, or only referenced at import time?
...
3. Should FideliOS support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps FideliOS aligned with:
