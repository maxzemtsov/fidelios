...
Today release smoke testing for published FideliOS packages is manual and shell-driven:
...
- runs `fidelios@${FIDELIOS_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@fidelios.local`
- password: `fidelios-smoke-password`
...
1. A published `fidelios@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `fidelios@latest` Docker install can be smoke-tested by Playwright in CI.
