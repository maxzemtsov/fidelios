...
Add a single storage subsystem for FideliOS that supports:
...
- API serves attachment bytes through authenticated FideliOS endpoints.
...
- Default local path is under instance root: `~/.fidelios/instances/<instanceId>/data/storage`.
...
- `fidelios onboard` writes a valid `storage` config block by default.
- `fidelios configure --section storage` can switch between local and s3 modes.
...
- `fidelios doctor` reports actionable storage status.
