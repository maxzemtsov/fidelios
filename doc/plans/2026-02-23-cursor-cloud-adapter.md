...
This document defines the V1 design for a FideliOS adapter that integrates with
...
4. synthesized stdout events for FideliOS UI/CLI
...
3. **Agent callback auth to FideliOS** uses a bootstrap exchange flow (no long-lived FideliOS key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from FideliOS endpoints, not full SKILL.md prompt injection.
...
- `fideliosPublicUrl` (optional override; else `FIDELIOS_PUBLIC_URL` env)
...
## FideliOS Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `FIDELIOS_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.fideliosPublicUrl`
2. `process.env.FIDELIOS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived FideliOS credentials in prompt text.

Flow:

1. Before launch/follow-up, FideliOS mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. FideliOS validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all FideliOS API calls.
...
1. Prompt includes a compact instruction to fetch skills from FideliOS.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/fidelios`
   - `GET /api/skills/fidelios-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <fideliosPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active FideliOS run context.
...
5. `fideliosPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from FideliOS skill endpoints |
| FideliOS auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from FideliOS context.
3. Add richer PR metadata surfacing in FideliOS UI.
