...
`PAP-447` asks how FideliOS should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same FideliOS instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into FideliOS:

- some operators use FideliOS to manage FideliOS and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as FideliOS
- Claude and Codex expose different built-in affordances, so FideliOS should not overfit to one tool

## Core Product Decision

FideliOS should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by FideliOS
...
- FideliOS should not assume that every adapter can see or use a host filesystem path directly
...
If FideliOS makes worktrees agent-first, it will blur:
...
- FideliOS core resolves and records execution workspace state
...
That gives FideliOS room to support:
...
- FideliOS handles workspace runtime services under the hood using project/workspace policy
...
Once FideliOS is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So FideliOS should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that FideliOS cannot infer
...
- when issue moves to done, should FideliOS auto-commit?
- should it auto-push?
...
FideliOS already has the right foundation for a project-first model.
...
For server-spawned local adapters, FideliOS already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that FideliOS should mirror directly

Implication:

- for `codex_local`, FideliOS should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but FideliOS should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, FideliOS should separate:
...
FideliOS should be able to express intentions such as:
...
These are not specific to FideliOS. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".fidelios/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a FideliOS-specific `devServer` flag.
...
FideliOS should distinguish between:
...
FideliOS should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, FideliOS should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to FideliOS
...
For runtime services specifically, FideliOS should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as FideliOS-only dev-server flags
...
3. Inject resolved execution workspace details into `context.fideliosWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / FideliOS instance can be reused through env/config injection when appropriate
...
- FideliOS can decide whether to reuse or start a fresh service deterministically
...
3. Test against FideliOS-on-FideliOS workflows first.
...
7. Workspace runtime services are modeled generically, not as FideliOS-specific dev-server toggles.
