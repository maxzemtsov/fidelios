...
FideliOS needs to support two very different but equally valid ways of working:
...
Today, FideliOS already has the beginnings of this model:
...
- making FideliOS feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing FideliOS users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted FideliOS deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning FideliOS into a full code review product
...
This object must be recorded explicitly so that FideliOS can:
...
FideliOS should treat PRs as a type of work product linked back to:
...
FideliOS already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted FideliOS must be a first-class deployment mode

FideliOS cannot assume that it is running on the same machine as the code.

In cloud deployments, FideliOS may:
...
- `Runtime service`: a process or service FideliOS owns or tracks for a workspace
...
It also lets cloud-hosted FideliOS deployments point at codebases and remotes without pretending the FideliOS host has direct filesystem access.
...
Without an explicit `execution workspace` record, FideliOS has nowhere stable to attach:
...
FideliOS needs a single place to show:
...
- FideliOS-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without FideliOS owning the execution host
...
## 1. FideliOS host and execution host must be decoupled
...
without the FideliOS host itself running local git or local preview processes.
...
FideliOS should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for FideliOS.
...
FideliOS should keep workspace automation centered on a small set of hooks:
...
FideliOS should provide the equivalent visibility in the project `Code` surface:
...
FideliOS should treat execution workspaces as the natural home for:
...
FideliOS should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
FideliOS should continue treating cleanup policy as part of the core workspace model:
...
FideliOS should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the FideliOS host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

FideliOS should borrow Worktrunk's good execution patterns while keeping the broader FideliOS model:
...
That keeps FideliOS focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted FideliOS deployments remain first-class

That is a better fit for FideliOS than either extreme:
