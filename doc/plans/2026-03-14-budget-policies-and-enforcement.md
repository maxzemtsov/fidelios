...
FideliOS already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model FideliOS should implement next.

## Product Goals

FideliOS should let operators:
...
For the next implementation pass, FideliOS should enforce these defaults:
...
FideliOS should separate subscription-included usage from billed spend:
...
FideliOS should have two threshold classes:
...
FideliOS must know that a project is budget-paused and enforce it.
...
FideliOS must also block execution before new work starts.
