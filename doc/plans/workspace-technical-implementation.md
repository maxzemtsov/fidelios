...
This repo already uses `FIDELIOS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `FIDELIOS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - FideliOS may create host-local worktrees, processes, and paths
- `cloud`
  - FideliOS should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `fidelios | github | vercel | s3 | custom`
...
- use `FIDELIOS_EXECUTION_TOPOLOGY` as a defaulting guardrail
