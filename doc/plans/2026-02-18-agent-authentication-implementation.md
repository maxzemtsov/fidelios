...
- New config/env settings:
  - `FIDELIOS_AGENT_JWT_SECRET`
  - `FIDELIOS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `FIDELIOS_AGENT_JWT_ISSUER` (default: `fidelios`)
  - `FIDELIOS_AGENT_JWT_AUDIENCE` (default: `fidelios-api`)
...
1. In:
   ...
   inject `FIDELIOS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `FIDELIOS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `FIDELIOS_AGENT_ID`
  - `FIDELIOS_COMPANY_ID`
  - `FIDELIOS_API_URL`
...
- Local adapters authenticate without manual `FIDELIOS_API_KEY` config.
