# FideliOS Memory Service Plan

## Goal

Define a FideliOS memory service and surface API that can sit above multiple memory backends, while preserving FideliOS's control-plane requirements:

- company scoping
- auditability
- provenance back to FideliOS work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current FideliOS architecture in:
...
FideliOS should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing FideliOS plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. FideliOS owns routing, provenance, and accounting

Providers should not decide how FideliOS entities map to governance.

FideliOS core should own:
...
The normalized FideliOS scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through FideliOS.

FideliOS should log every operation, whether the provider is local or external.
...
## What FideliOS Should Persist

FideliOS should not mirror the full provider memory corpus into Postgres unless the provider is a FideliOS-managed local provider.

FideliOS core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, FideliOS may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, FideliOS may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, FideliOS may capture selected issue comments or issue documents as memory sources.
...
FideliOS should give agents both automatic recall and explicit tools, with simple guidance:
...
FideliOS needs a first-class UI for memory, otherwise providers become black boxes.
...
This is important because a memory system that "works" but silently burns budget is not acceptable in FideliOS.
...
- it matches FideliOS's local-first posture
- it is inspectable
...
- Do we want FideliOS-managed extraction pipelines at all, or should built-ins be the only place where FideliOS owns extraction?
...
- FideliOS owns memory bindings, scopes, provenance, governance, and usage reporting.
