...
This document defines the product and UI plan for skill management in FideliOS.
...
- FideliOS company import/export can include skills as package content
...
3. The UI must always tell the truth about what FideliOS knows:
   - desired state in FideliOS
...
FideliOS should treat skills at two scopes:
...
- created locally inside FideliOS later
...
7. “Understand what FideliOS knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a FideliOS-specific package layout
...
This matters because Codex and similar adapters may already have local skills that FideliOS did not install.

These should be clearly marked:

- external
- not managed by FideliOS
...
- `desired_only`: FideliOS wants it, adapter does not show it yet
- `external`: adapter has it but FideliOS does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, FideliOS only tracks desired state
...
- FideliOS can still track desired skills
...
- FideliOS can see adapter skills
...
- `.fidelios.yaml` may add FideliOS-specific fidelity, but should not replace the base shortname association model
...
- `.fidelios.yaml` may add FideliOS-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
FideliOS should have a company-scoped skill model or managed package model representing:
...
FideliOS should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too FideliOS-specific.
