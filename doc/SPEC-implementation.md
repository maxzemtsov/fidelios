...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.fidelios/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.fidelios/instances/default/data/storage` (`local_disk`)
...
- `.fidelios.yaml` sidecar for FideliOS-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.fidelios.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and FideliOS routine fidelity in `.fidelios.yaml`
- FideliOS imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.fidelios.yaml`
