...
For `claude_local` and `codex_local`, FideliOS assumes the CLI is already installed and authenticated on the host machine.
...
FideliOS stores resumable session state per `(agent, taskKey, adapterType)`.
