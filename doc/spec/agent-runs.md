...
This spec defines how FideliOS actually runs agents while staying runtime-agnostic.
...
1. FideliOS is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, FideliOS manages lifecycle, and on exit FideliOS parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `fidelios.skill` (shared FideliOS skill text block)
...
2. Preferred transport is env vars (`FIDELIOS_*`) injected at runtime.
