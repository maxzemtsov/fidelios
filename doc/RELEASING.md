...
Maintainer runbook for shipping FideliOS across npm, GitHub, and the website-facing changelog surface.
...
FideliOS uses calendar versions that still fit semver syntax:
...
2. **npm** — `fidelios` and public workspace packages are published
...
# or
npx fidelios@canary onboard --data-dir "$(mktemp -d /tmp/fidelios-canary.XXXXXX)"
```
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for FideliOS. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
FIDELIOS_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable FIDELIOS_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f fidelios_version=canary
gh workflow run release-smoke.yml -f fidelios_version=latest
```

Minimum checks:

- `npx fidelios@canary onboard` installs
