...
Build a custom adapter to connect FideliOS to any agent runtime.
...
2. Build environment with `buildFideliOSEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `fidelios run --watch` using `picocolors`.
...
Make FideliOS skills discoverable to your agent runtime without writing to the agent's working directory:
