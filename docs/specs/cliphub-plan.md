# ClipHub: Marketplace for FideliOS Team Configurations
...
> The "app store" for whole-company AI teams — pre-built FideliOS configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for FideliOS-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | FideliOS company (agents, projects, governance) |
...
A complete FideliOS company configuration:
...
Single-agent configurations designed to plug into a FideliOS org:
...
Portable skill files that any FideliOS agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with FideliOS's skill loading system
...
interface Listing {
  ...
  fideliosVersionMin: string;     // Minimum FideliOS version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target FideliOS company
...
| `POST` | `/api/listings/:id/install` | Install to FideliOS company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target FideliOS company (or create new)
  → ClipHub API calls FideliOS API to:
  ...
  → Redirect to FideliOS dashboard with new team running
```
...
### 5.3 Creator: Export from FideliOS → Publish

```
Running FideliOS company → "Export as Blueprint" (CLI or UI)
  → FideliOS exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (FideliOS brand blue/purple)
...
| Install flow | One-click deploy to FideliOS company |
...
{
  "targetCompanyId": "uuid",         // Existing FideliOS company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `fidelios-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as FideliOS
- **Backend**: Node.js API (or extend FideliOS server)
- **Database**: Postgres (can share FideliOS's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with FideliOS auth (or OAuth2)

### 9.2 Integration with FideliOS

ClipHub can be:
- **Option A**: A separate app that calls FideliOS's API to install blueprints
- **Option B**: A built-in section of the FideliOS UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing FideliOS UI and API.
...
- [ ] Install flow: blueprint → FideliOS company
...
- [ ] "Export from FideliOS" CLI command
