...
How to get OpenClaw running in a Docker container for local development and testing the FideliOS OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

FideliOS includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-fidelios-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a FideliOS host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-fidelios-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `FIDELIOS_HOST_PORT` (default `3100`)
- `FIDELIOS_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your FideliOS deployment is `authenticated`, provide auth context:

```bash
FIDELIOS_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
FIDELIOS_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host FideliOS server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed FideliOS URL (typically `http://host.docker.internal:3100`).
- If FideliOS rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm fidelios allowed-hostname host.docker.internal
```

Then restart FideliOS and rerun the smoke script.
