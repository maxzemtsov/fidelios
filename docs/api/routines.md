...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-FideliOS-Signature` + `X-FideliOS-Timestamp` header pair matching the trigger's signing mode.
