...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only FideliOS.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.fidelios.yaml`
...
- FideliOS must not require extra top-level fields for skill validity
- FideliOS-specific extensions must live under `metadata.fidelios` or `metadata.sources`
...
For FideliOS, the preferred fidelity extension is:
...
- issue/task FideliOS-only metadata
...
- FideliOS tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested FideliOS shape:
...
Additional rules for FideliOS exporters:
...
- omit empty and default-valued FideliOS fields when possible
...
- FideliOS exporters should emit `.fidelios.yaml` as a sidecar by default
...
FideliOS can map this spec to its runtime model like this:
...
- FideliOS extension:
  - `.fidelios.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other FideliOS-specific fidelity

Inline FideliOS-only metadata that must live inside a shared markdown file should use:

- `metadata.fidelios`

That keeps the base format broader than FideliOS.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only FideliOS.

## 20. Cutover

FideliOS should cut over to this markdown-first package model as the primary portability format.

`fidelios.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For FideliOS, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
