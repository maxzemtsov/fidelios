...
Audience: Operators setting up and running agents in FideliOS
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`), FideliOS assumes the CLI is already installed and authenticated on the host machine.
...
FideliOS stores session IDs for resumable adapters.
...
FideliOS pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. FideliOS surfaces this as a warning in environment tests, not a hard error.
