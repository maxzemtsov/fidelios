...
FIDELIOS_DEPLOYMENT_MODE="${FIDELIOS_DEPLOYMENT_MODE:-authenticated}"
FIDELIOS_DEPLOYMENT_EXPOSURE="${FIDELIOS_DEPLOYMENT_EXPOSURE:-private}"
FIDELIOS_PUBLIC_URL="${FIDELIOS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@fidelios.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-fidelios-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e FIDELIOS_DEPLOYMENT_MODE="$FIDELIOS_DEPLOYMENT_MODE" \
      -e FIDELIOS_DEPLOYMENT_EXPOSURE="$FIDELIOS_DEPLOYMENT_EXPOSURE" \
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$FIDELIOS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$FIDELIOS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$FIDELIOS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$FIDELIOS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$FIDELIOS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$FIDELIOS_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $FIDELIOS_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $FIDELIOS_DEPLOYMENT_MODE/$FIDELIOS_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/fidelios-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$FIDELIOS_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $FIDELIOS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$FIDELIOS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $FIDELIOS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
