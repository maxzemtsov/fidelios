...
FIDELIOS_API_URL="${FIDELIOS_API_URL:-http://127.0.0.1:3100}"
API_BASE="${FIDELIOS_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-fidelios-smoke}"
...
FIDELIOS_API_URL_FOR_OPENCLAW="${FIDELIOS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${FIDELIOS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${FIDELIOS_AUTH_HEADER}" )
fi
if [[ -n "${FIDELIOS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${FIDELIOS_COOKIE}" )
  FIDELIOS_BROWSER_ORIGIN="${FIDELIOS_BROWSER_ORIGIN:-${FIDELIOS_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${FIDELIOS_BROWSER_ORIGIN}" -H "Referer: ${FIDELIOS_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set FIDELIOS_COOKIE or FIDELIOS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg fideliosApiUrl "$FIDELIOS_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/fidelios"
  local claimed_file="${workspace_dir}/fidelios-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/fidelios-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/fidelios-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/fidelios-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/fidelios"
    assert_status "200"
    {
      echo "FIDELIOS_API_URL=${FIDELIOS_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set FIDELIOS_API_KEY=${AGENT_API_KEY} in your run context before running FideliOS heartbeat steps."
...
main() {
  ...
  wait_http_ready "${FIDELIOS_API_URL%/}/api/health" 15 || fail "FideliOS API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "fidelios health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
