...
FIDELIOS_HOST_PORT="${FIDELIOS_HOST_PORT:-3100}"
FIDELIOS_HOST_FROM_CONTAINER="${FIDELIOS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.fidelios-openclaw.override.yml"
...
detect_fidelios_base_url() {
  ...
  for candidate in "$FIDELIOS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${FIDELIOS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${FIDELIOS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
fidelios_base_url="$(detect_fidelios_base_url || true)"
...
FideliOS URL for OpenClaw container:
EOF
  if [[ -n "$fidelios_base_url" ]]; then
    cat <<EOF
  $fidelios_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${FIDELIOS_HOST_PORT}
  (Do not use http://127.0.0.1:${FIDELIOS_HOST_PORT} inside the container.)
  If FideliOS rejects the host, run on host machine:
    pnpm fidelios allowed-hostname host.docker.internal
  Then restart FideliOS and re-run this script.
EOF
  fi
  cat <<EOF
