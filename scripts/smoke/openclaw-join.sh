...
COMPANY_ID="${COMPANY_ID:-${FIDELIOS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-fidelios-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-fidelios-openclaw-smoke}"
...
Provide one of:
  FIDELIOS_AUTH_HEADER=\"Bearer <board-token>\"
  FIDELIOS_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "FideliOS OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.fidelios.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
