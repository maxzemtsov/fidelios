#!/usr/bin/env bash
#
# Kill all local FideliOS dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all fidelios dev processes
...
# Collect PIDs of node processes running from any fidelios directory.
# Matches paths like /Users/*/fidelios/... or /Users/*/fidelios-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/fidelios(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No FideliOS dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} FideliOS dev process(es):"
echo ""
