...
Environment overrides:
  ...
  PC_DATA=/tmp/data           FideliOS data dir to use
  FIDELIOS_HOST=127.0.0.1    Host passed to the onboarded server
  FIDELIOS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/fidelios-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/fidelios-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export FIDELIOS_HOST="${FIDELIOS_HOST:-127.0.0.1}"
export FIDELIOS_PORT="${FIDELIOS_PORT:-3100}"
export FIDELIOS_OPEN_ON_LISTEN="${FIDELIOS_OPEN_ON_LISTEN:-false}"
...
echo "FIDELIOS_HOST: $FIDELIOS_HOST"
echo "FIDELIOS_PORT: $FIDELIOS_PORT"
...
env \
  ...
  HOST="$FIDELIOS_HOST" \
  PORT="$FIDELIOS_PORT" \
  FIDELIOS_OPEN_ON_LISTEN="$FIDELIOS_OPEN_ON_LISTEN" \
  pnpm fidelios onboard --yes --data-dir "$PC_DATA"
