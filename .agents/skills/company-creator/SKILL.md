...
description: >
  ...
  is already running in FideliOS.
---
...
└── .fidelios.yaml            (FideliOS vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .fidelios.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [FideliOS](https://github.com/maxzemtsov/fidelios)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to FideliOS (https://github.com/maxzemtsov/fidelios)
- A "Getting Started" section explaining how to import: `fidelios company import --from <path>`
...
## .fidelios.yaml Guidelines

The `.fidelios.yaml` file is the FideliOS vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — FideliOS will use its default. Specifying an unknown adapter type causes an import error.

FideliOS's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.fidelios.yaml`.
